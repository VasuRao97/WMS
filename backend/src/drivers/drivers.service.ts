...
export class DriversService {
  ...
  private validate(data: any, errors: string[]) {
    ...
    if (data.isBlacklisted && !data.blacklistReason?.trim()) {
      errors.push(
        'Blacklist Reason is required when marking a driver blacklisted.',
      );
    }
  }
  ...
  private async resolveWarehouseId(
    ...
    if (GATE_YARD_SCOPED_ROLES.includes(user.role)) {
      ...
      if (!ids.includes(warehouseId)) {
        errors.push(
          'You can only register a driver to your own assigned warehouse(s).',
        );
        return undefined;
      }
    }
  ...
  async create(data: any, user: AuthUser) {
    ...
    if (!user.companyId) {
      throw new ForbiddenException(
        'Super admin accounts cannot register drivers directly — log in as a company admin instead.',
      );
    }
    const errors: string[] = [];
  ...
  async removeAll(user: AuthUser) {
    ...
    const drivers = await this.prisma.driver.findMany({
      ...
      select: {
        id: true,
        name: true,
        _count: { select: { gateEntries: true } },
      },
    });
    const deletable = drivers
      .filter((d) => d._count.gateEntries === 0)
      .map((d) => d.id);
    const blocked = drivers
      .filter((d) => d._count.gateEntries > 0)
      .map((d) => d.name);
    if (deletable.length > 0)
      await this.prisma.driver.deleteMany({ where: { id: { in: deletable } } });
    return {
  ...
  async remove(id: string, user: AuthUser) {
    ...
    const count = await this.prisma.vehicleGateEntry.count({
      where: { driverId: id },
    });
    if (count > 0) {
      throw new BadRequestException(
        `Cannot permanently delete "${driver.name}" — it has ${count} linked gate entry record(s). Deactivate it instead.`,
      );
    }
