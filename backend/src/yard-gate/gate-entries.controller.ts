...
export class GateEntriesController {
  ...
  matchReceipt(
    @Param('id') id: string,
    @Body()
    body: { stagingLocationId: string; requiresPalletConsolidation?: boolean },
    @CurrentUser() user: AuthUser,
  ) {
  ...
  scan(
    @Param('id') id: string,
    @Body() body: { barcode: string; palletId?: string },
    @CurrentUser() user: AuthUser,
  ) {
    return this.gateEntriesService.scan(
      id,
      body?.barcode,
      user,
      body?.palletId,
