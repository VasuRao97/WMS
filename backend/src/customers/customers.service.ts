...
export class CustomersService {
  ...
  private validate(data: any): string[] {
    ...
    if (!data.code || !CODE_REGEX.test(data.code)) {
      errors.push(
        'Bill To ID is required: alphanumeric/hyphens only, max 30 characters.',
      );
    }
    ...
    if (data.shipToLocations && data.shipToLocations.length > 0) {
      ...
      for (const s of data.shipToLocations) {
        if (!s.address)
          errors.push('Each Ship-to location requires an address.');
        if (!s.pincode || !PINCODE_REGEX.test(s.pincode))
          errors.push(
            'Each Ship-to location requires a valid 6-digit pincode.',
          );
        if (!s.state) errors.push('Each Ship-to location requires a state.');
        if (s.gstNumber && !GST_REGEX.test(s.gstNumber))
          errors.push(`Ship-to GST number format is invalid: ${s.gstNumber}`);
        if (
          s.deliveryZone &&
          !DELIVERY_ZONE_VALUES.includes(
            String(s.deliveryZone).trim().toUpperCase(),
          )
        ) {
          errors.push(
            `Ship-to Local/Upcountry must be one of: ${DELIVERY_ZONE_VALUES.join(', ')}`,
          );
        }
        if (s.isDefault) defaultCount++;
      }
      if (defaultCount > 1)
        errors.push('Only one Ship-to location can be marked as Default.');
    }
    return errors;
  }

  private async resolveShipTos(
    shipToLocations: any[],
    user: AuthUser,
    errors: string[],
  ) {
    ...
    for (const s of shipToLocations || []) {
        ...
        const wh = await this.prisma.warehouse.findUnique({
          where: {
            companyId_code: {
              ...
              code: s.warehouseCode.toUpperCase(),
            },
          },
        });
        if (!wh) {
          errors.push(
            `Warehouse code "${s.warehouseCode}" not found for this company.`,
          );
          continue;
        }
        warehouseId = wh.id;
      ...
      resolved.push({
        ...
        deliveryZone: s.deliveryZone
          ? String(s.deliveryZone).toUpperCase().trim()
          : undefined,
      });
    }
    return resolved;
  ...
  async create(data: any, user: AuthUser) {
    if (!user.companyId) {
      throw new ForbiddenException(
        'Super admin accounts cannot create customers directly — log in as a company admin instead.',
      );
    }
    const errors = this.validate(data);
    const shipToCreateData = await this.resolveShipTos(
      data.shipToLocations,
      user,
      errors,
    );
    ...
    if (existing)
      throw new BadRequestException(
        `Bill To ID "${data.code}" already exists.`,
      );
    ...
    return this.prisma.customer.create({
      data: {
        ...
        shipToLocations: shipToCreateData.length
          ? { create: shipToCreateData }
          : undefined,
      },
      include: {
        shipToLocations: {
  ...
  async removeAll(user: AuthUser) {
    ...
    if (ids.length > 0) {
      await this.prisma.$transaction([
        this.prisma.customerShipTo.deleteMany({
  ...
  async bulkImport(groupedCustomers: any[], user: AuthUser) {
    if (!user.companyId) {
      throw new ForbiddenException(
        'Super admin accounts cannot import customers directly — log in as a company admin instead.',
      );
    }
    const results: any[] = [];
    ...
    for (const group of groupedCustomers) {
      ...
      if (errors.length === 0 && upperCode) {
        ...
        if (existing)
          errors.push(
            `Bill To ID already exists in the database: ${upperCode}`,
          );
      }
      ...
      try {
        ...
        results.push({
          code: upperCode,
          status: 'success',
          shipToCount: shipToCreateData.length,
        });
