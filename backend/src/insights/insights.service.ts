...
export class InsightsService {
  ...
  async storageUtilization(user: AuthUser, warehouseId: string) {
    ...
    const totals: Record<
      string,
      { lanesUsed: number; binsAllotted: number; binsUsed: number }
    > = {
    ...
    if (locations.length > 0) {
      ...
      for (const m of movements) {
        if (!balanceByLoc.has(m.locationId))
          balanceByLoc.set(m.locationId, new Map());
      ...
      const classBySkuId = new Map(
        skus.map((s) => [s.id, (s.abcClass || 'C').toUpperCase()]),
      );
      ...
      for (const laneLocations of lanes.values()) {
        const occupiedLocIds = laneLocations
          .filter((l) => occupantSkuIdsByLoc.has(l.id))
          .map((l) => l.id);
    ...
    return {
      ...
      classes: ABC_CLASSES.map((cls) => {
        ...
        return {
          ...
          utilizationPct:
            t.binsAllotted > 0
              ? Math.round((t.binsUsed / t.binsAllotted) * 1000) / 10
              : null,
        };
      }),
    };
