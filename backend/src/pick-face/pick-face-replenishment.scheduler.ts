...
export class PickFaceReplenishmentScheduler {
  ...
  async runDailyReslotting() {
    ...
    for (const warehouse of warehouses) {
      try {
        ...
        this.logger.error(
          `Pick Face reslotting failed for warehouse ${warehouse.id}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
  ...
  async reslotWarehouse(warehouseId: string) {
    ...
    for (const m of movements) {
      if (!balanceByLocation.has(m.locationId))
        balanceByLocation.set(m.locationId, new Map());
    ...
    const occupantByPickFaceLoc = new Map<
      string,
    ...
    for (const o of occupantByPickFaceLoc.values())
      alreadySlottedSkuIds.add(o.skuId);
    for (const t of openTasks)
      if (t.reason === 'REFILL') alreadySlottedSkuIds.add(t.skuId);
    const targetedToLocationIds = new Set(openTasks.map((t) => t.toLocationId));
    const targetedFromLocationIds = new Set(
      openTasks
        .filter((t) => t.reason === 'EVICTION')
        .map((t) => t.fromLocationId),
    );
    ...
    const occupantSkuIds = [...occupantByPickFaceLoc.values()].map(
      (o) => o.skuId,
    );
    const candidateSkuIds = [...reserveBySku.keys()].filter(
      (id) => !alreadySlottedSkuIds.has(id),
    );
    const relevantSkus = await this.prisma.sku.findMany({
      where: {
        id: { in: [...new Set([...occupantSkuIds, ...candidateSkuIds])] },
      },
      select: { id: true, code: true, abcClass: true },
    });
    const classById = new Map(
      relevantSkus.map((s) => [s.id, (s.abcClass || 'C').toUpperCase()]),
    );
    ...
    const candidateQueue = relevantSkus
      .filter(
        (s) =>
          candidateSkuIds.includes(s.id) &&
          ['A', 'B'].includes((s.abcClass || '').toUpperCase()),
      )
      .map((s) => ({
        id: s.id,
        code: s.code,
        abcClass: (s.abcClass || '').toUpperCase(),
      }))
      .sort(
        (a, b) =>
          rank(a.abcClass) - rank(b.abcClass) || a.code.localeCompare(b.code),
      );

    // Stable location processing order for determinism.
    const sortedLocations = [...pickFaceLocations].sort((a, b) =>
    ...
    for (const loc of sortedLocations) {
      ...
      if (!occupant) {
        ...
        await this.createRefillTask(
          warehouseId,
          loc.id,
          candidate.id,
          reserveBySku.get(candidate.id)!,
        );
        continue;
      }
  ...
  // arbitrary one.
  private async createRefillTask(
    warehouseId: string,
    toLocationId: string,
    skuId: string,
    reserveOptions: { locationId: string; qty: number }[],
  ) {
    const leanest = reserveOptions.reduce((min, cur) =>
      cur.qty < min.qty ? cur : min,
    );
  ...
  private async createEvictionTask(
    warehouseId: string,
    fromLocationId: string,
    skuId: string,
    qty: number,
  ) {
    const toLocationId = await this.putawayTasks.suggestBin(this.prisma, {
      warehouseId,
      skuId,
    });
