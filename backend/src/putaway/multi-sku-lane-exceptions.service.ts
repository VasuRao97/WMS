...
export class MultiSkuLaneExceptionsService {
  ...
  async create(data: any, user: AuthUser) {
    ...
    if (!scopedIds.includes(data.warehouseId))
      throw new ForbiddenException(
        'You can only request this for your own assigned warehouse(s).',
      );

    const existingActive = await this.prisma.multiSkuLaneException.findFirst({
      where: {
    ...
    if (existingActive)
      throw new BadRequestException(
        `This warehouse already has ${existingActive.status === 'PENDING' ? 'a pending request' : 'an active exception'} — resolve it before submitting a new one.`,
      );
  ...
  async findAll(user: AuthUser, warehouseId?: string) {
    ...
    return this.prisma.multiSkuLaneException.findMany({
      where,
      include: EXCEPTION_INCLUDE,
  ...
  private async assertAccess(id: string, user: AuthUser) {
    const exception = await this.prisma.multiSkuLaneException.findUnique({
      where: { id },
    ...
    if (
      ...
      exception.warehouse.companyId !== user.companyId
    )
  ...
  async approve(id: string, reviewNote: any, user: AuthUser) {
  ...
  async reject(id: string, reviewNote: any, user: AuthUser) {
    const exception = await this.assertAccess(id, user);
    if (exception.status !== 'PENDING')
  ...
  async revoke(id: string, user: AuthUser) {
    ...
    if (exception.status !== 'APPROVED')
      throw new BadRequestException(
        'Only an approved exception can be revoked.',
      );
    return this.prisma.multiSkuLaneException.update({
      where: { id },
      data: {
