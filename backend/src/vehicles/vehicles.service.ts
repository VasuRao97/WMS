...
export class VehiclesService {
  ...
  private validate(data: any, errors: string[]) {
    ...
    if (data.isBlacklisted && !data.blacklistReason?.trim()) {
      errors.push(
        'Blacklist Reason is required when marking a vehicle blacklisted.',
      );
    }
  ...
  private async resolveWarehouseId(
    ...
    if (GATE_YARD_SCOPED_ROLES.includes(user.role)) {
      ...
      if (!ids.includes(warehouseId)) {
        errors.push(
          'You can only register a vehicle to your own assigned warehouse(s).',
        );
        return undefined;
      }
    }
  ...
  async create(data: any, user: AuthUser) {
    ...
    if (!user.companyId) {
      throw new ForbiddenException(
        'Super admin accounts cannot register vehicles directly — log in as a company admin instead.',
      );
    }
    const errors: string[] = [];
    ...
    const existing = await this.prisma.vehicle.findUnique({
      where: {
        companyId_vehicleNumber: { companyId: user.companyId, vehicleNumber },
      },
    });
    if (existing)
      throw new BadRequestException(
        `Vehicle Number "${vehicleNumber}" is already registered.`,
      );
    ...
    return this.prisma.vehicle.create({
      data: {
        ...
        blacklistReason: data.isBlacklisted
          ? String(data.blacklistReason).trim()
          : undefined,
      },
      include: {
        vehicleType: {
  ...
  async findAll(user: AuthUser, warehouseId?: string) {
    await assertGateAccessAllowed(this.prisma, user);
    const accessibleIds = await gateYardAccessibleWarehouseIds(
      this.prisma,
      user,
    );
  ...
  async exportRows(user: AuthUser) {
    ...
    return vehicles.map((v) => ({
      ...
      'Detention Cost/Day (₹)':
        v.detentionCostPerDay ?? v.vehicleType.detentionCostPerDay ?? '',
      ...
      'Insurance Expiry': v.insuranceExpiry
        ? v.insuranceExpiry.toISOString().slice(0, 10)
        : '',
      ...
      'Fitness Expiry': v.fitnessExpiry
        ? v.fitnessExpiry.toISOString().slice(0, 10)
        : '',
  ...
  async update(id: string, data: any, user: AuthUser) {
    ...
    return this.prisma.vehicle.update({
      ...
      data: {
        ...
        detentionCostPerDay:
          toNumberOrUndefined(data.detentionCostPerDay) ?? null,
  ...
  async removeAll(user: AuthUser) {
    ...
    const vehicles = await this.prisma.vehicle.findMany({
      ...
      // follow-up), outboundOrders added 2026-09-14 (Outbound design) — a
      // real lesson this codebase already learned twice now with
    ...
    const deletable = vehicles
      .filter(
        (v) =>
          v._count.gateEntries === 0 &&
          v._count.inboundReceipts === 0 &&
          v._count.outboundOrders === 0,
      )
      .map((v) => v.id);
  ...
  async remove(id: string, user: AuthUser) {
    ...
    const [gateEntryCount, receiptCount, outboundOrderCount] =
      await Promise.all([
        this.prisma.vehicleGateEntry.count({ where: { vehicleId: id } }),
        this.prisma.inboundReceipt.count({ where: { vehicleId: id } }),
        this.prisma.outboundOrder.count({ where: { vehicleId: id } }),
      ]);
    if (gateEntryCount > 0 || receiptCount > 0 || outboundOrderCount > 0) {
      const parts = [
        gateEntryCount > 0
          ? `${gateEntryCount} linked gate entry record(s)`
          : null,
        receiptCount > 0 ? `${receiptCount} linked inbound order(s)` : null,
        outboundOrderCount > 0
          ? `${outboundOrderCount} linked outbound order(s)`
          : null,
      ].filter(Boolean);
      throw new BadRequestException(
        `Cannot permanently delete "${vehicle.vehicleNumber}" — it has ${parts.join(' and ')}. Deactivate it instead.`,
      );
    }
